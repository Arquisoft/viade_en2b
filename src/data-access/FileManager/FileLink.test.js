...
const mockItemExists = jest.fn().mockImplementation(async (routeName) => {
  return routeName === "https://example.org/viade/routes/Oviedo.jsonld";
});
...
const mockReadFolder = jest.fn().mockImplementation(async (folder) => {
  return {
    files: [
      {
        name: "Oviedo.jsonld",
        url: "https://example.org/viade/routes/Oviedo.jsonld",
